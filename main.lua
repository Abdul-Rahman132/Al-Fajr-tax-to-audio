...
local EMOTIONS = {
    ...
    "News Anchor", "Narrator", "Formal", "Casual", "Friendly", "Cheerful",
    "Hopeful", "Playful", "Romantic", "Loving", "Gentle", "Humble", "Innocent",
    "Calm", "Confident", "Proud", "Strong", "Bold", "Passionate", "Gloomy",
    "Desperate", "Shy", "Bored", "Surprised", "Sarcastic", "Storytelling",
    "Dramatic", "Energetic", "Funny", "Mysterious", "Serious", "Whisper", "Fast", "Slow"
}
...
function closeAndCleanup()
    if currentHttpRequest ~= nil then 
        pcall(function() currentHttpRequest.cancel() end) 
...
function vibrate(ms) 
    ...
    if vibrator then pcall(function() vibrator.vibrate(ms or 50) end) end 
end
...
function saveSettings()
    ...
    editor.putInt("podcast_gen_mode", podcastGenerationMode)
    editor.apply()
end
...
function loadSettings()
    ...
    podcastGenerationMode = prefs.getInt("podcast_gen_mode", 1)
end
...
function initMediaPlayer()
    ...
    mediaPlayer.setOnCompletionListener(MediaPlayer.OnCompletionListener({
        onCompletion = function(mp)
            isPlaying = false
            if currentPlayBtn then runOnUi(function() currentPlayBtn.setText("PLAY") end) end
        end
    }))
end
...
function writeWavHeader(outStream, totalAudioLen, sampleRate)
    ...
    local function writeShort(stream, val)
        stream.write(val & 0xFF)
        stream.write((val >> 8) & 0xFF)
    end
    local function writeInt(stream, val)
        stream.write(val & 0xFF)
        stream.write((val >> 8) & 0xFF)
    ...
    outStream.write(0x52); outStream.write(0x49); outStream.write(0x46); outStream.write(0x46)
    writeInt(outStream, totalSize)
    outStream.write(0x57); outStream.write(0x41); outStream.write(0x56); outStream.write(0x45)
    outStream.write(0x66); outStream.write(0x6D); outStream.write(0x74); outStream.write(0x20)
    writeInt(outStream, 16)
    writeShort(outStream, 1)
    writeShort(outStream, channels)
    writeInt(outStream, sRate)
    writeInt(outStream, byteRate)
    writeShort(outStream, blockAlign)
    writeShort(outStream, bitsPerSample)
    outStream.write(0x64); outStream.write(0x61); outStream.write(0x74); outStream.write(0x61)
    writeInt(outStream, totalAudioLen)
end

function downloadWithCurrentSpeed()
    if not generatedAudioPath then showToast("Generate audio first") return false end
    ...
    if success then
        ...
        showToast("Download failed - storage permission?")
        return false
    end
end

-- Simple audio fetch
local function fetchAudioSimple(line, voice, apikey, model, callback)
    ...
    local req = Http.post(apiUrl, cjson.encode(requestBody), headers, function(code, content)
        if code == 200 then
                ...
                if candidate and candidate.content and candidate.content.parts then
                    for _, part in ipairs(candidate.content.parts) do
                        if part.inlineData and part.inlineData.data then
                            ...
                            callback(true, audioBytes, nil)
                            return
                        end
                    end
                end
            end
            callback(false, nil, "No audio in response")
        else
            callback(false, nil, "HTTP " .. code)
        end
    end)
    currentHttpRequest = req
end

-- ========== SIMPLIFIED PODCAST GENERATION (NO CHUNKING) ==========
function generatePodcast(userText, apikey, hostVoice, guestVoice, model, generateBtn, playBtn, resultLayout)
    -- Trim text if too long
    if #userText > MAX_CHARS then
        userText = userText:sub(1, MAX_CHARS)
        showToast("Text trimmed to " .. MAX_CHARS .. " chars")
    end
    
    local systemPrompt = "Convert the following text into a natural podcast conversation between 'Host' and 'Guest'. The conversation must alternate between Host and Guest, starting with Host, and cover ALL of the provided text. Output ONLY a valid JSON array of objects with keys 'speaker' ('host' or 'guest') and 'text'. No markdown, no extra text. Text: "
    ...
    local function startAudioGeneration(dialogue)
        if #dialogue == 0 then
            runOnUi(function()
                if generateBtn then generateBtn.setEnabled(true) end
                showToast("No dialogue generated, using Direct mode")
                generatePodcastDirect(userText, apikey, hostVoice, guestVoice, model, generateBtn, playBtn, resultLayout)
            end)
            return
        end
        ...
        local function processNextSegment()
            if index > #dialogue then
                if #allAudio == 0 then
                    runOnUi(function()
                        ...
                        if appHidden then appHidden=false; hiddenGenerating=false; showMain() end
                    end)
                    isGenerating = false
                    return
                end
                ...
                runOnUi(function()
                    if appHidden then
                        ...
                        showToast("Podcast ready! (" .. #dialogue .. " segments)")
                    end
                end)
                isGenerating = false
                return
            ...
            local speakerLower = (turn.speaker or ""):lower()
            local voice = (speakerLower == "host") and hostVoice or guestVoice
            local speakerName = (speakerLower == "host") and "Host" or "Guest"
            runOnUi(function() 
                if generateBtn then generateBtn.setText("AI: " .. speakerName .. " " .. index .. "/" .. #dialogue) end 
            end)
            
            fetchAudioSimple(turn.text, voice, apikey, model, function(success, audio, errMsg)
                if success then
                    table.insert(allAudio, audio)
                    index = index + 1
                    delay(1000, processNextSegment)
                else
                    runOnUi(function() showToast("Segment " .. index .. " failed: " .. (errMsg or "unknown") .. " - skipping") end)
                    index = index + 1
                    delay(1000, processNextSegment)
                end
            end)
    ...
    -- Make script request with simple retry
    local function attemptScript(attempt)
        attempt = attempt or 1
        currentHttpRequest = Http.post(scriptUrl, cjson.encode(scriptBody), scriptHeaders, function(code, content)
            currentHttpRequest = nil
            if code == 200 then
                local ok, data = pcall(cjson.decode, content)
                if ok and data and data.candidates and #data.candidates > 0 then
                ...
                if attempt < 3 then
                    runOnUi(function() showToast("Invalid script, retrying (" .. attempt .. "/3)") end)
                    delay(3000, function() attemptScript(attempt+1) end)
                else
                    runOnUi(function() showToast("Invalid script, using Direct mode") end)
                    generatePodcastDirect(userText, apikey, hostVoice, guestVoice, model, generateBtn, playBtn, resultLayout)
                end
            elseif code == 429 and attempt < 3 then
                runOnUi(function() showToast("Rate limit, retrying (" .. attempt .. "/3)") end)
                delay(5000, function() attemptScript(attempt+1) end)
            elseif attempt < 3 then
                runOnUi(function() showToast("Script error " .. code .. ", retrying (" .. attempt .. "/3)") end)
                delay(3000, function() attemptScript(attempt+1) end)
            else
                runOnUi(function()
                    if generateBtn then generateBtn.setEnabled(true) end
                    showToast("Script failed, using Direct mode")
                    generatePodcastDirect(userText, apikey, hostVoice, guestVoice, model, generateBtn, playBtn, resultLayout)
                end)
            end
        end)
...
-- Improved sentence splitting function (for direct mode)
function splitIntoSentences(text)
    local sentences = {}
    for sentence in text:gmatch("([^%.%?!]+[%.%?!]?%s*)") do
        local s = sentence:gsub("^%s*", ""):gsub("%s*$", "")
        if #s > 0 then table.insert(sentences, s) end
    end
    if #sentences >= 2 then return sentences end
    ...
    -- Fallback: split by words into chunks of ~100 chars
    sentences = {}
    local words = {}
    ...
    for i, w in ipairs(words) do
        if #chunk + #w + 1 > 100 and #chunk > 0 then
            table.insert(sentences, chunk)
            chunk = w
        else
            if #chunk > 0 then chunk = chunk .. " " .. w else chunk = w end
        end
    end
    if #chunk > 0 then table.insert(sentences, chunk) end
    if #sentences == 0 then table.insert(sentences, text) end
    return sentences
end

-- Direct Read Mode (No AI)
function generatePodcastDirect(originalText, apikey, hostVoice, guestVoice, model, generateBtn, playBtn, resultLayout)
    local sentences = splitIntoSentences(originalText)
    local dialogue = {}
    for i, sent in ipairs(sentences) do
        local speaker = (i % 2 == 1) and "host" or "guest"
        table.insert(dialogue, {speaker = speaker, text = sent})
    end
    
    local allAudio = {}
    local index = 1
    local function processSegment()
        if index > #dialogue then
            if #allAudio == 0 then
                runOnUi(function()
                    if generateBtn then generateBtn.setEnabled(true) end
            ...
            local totalLen = 0
            for _, chunk in ipairs(allAudio) do totalLen = totalLen + #chunk end
            local tempPath = context.getCacheDir().getPath() .. "/podcast_direct_" .. os.time() .. ".wav"
            local fos = FileOutputStream(File(tempPath))
            writeWavHeader(fos, totalLen, 24000)
            for _, chunk in ipairs(allAudio) do fos.write(chunk) end
            fos.close()
            ...
            runOnUi(function()
                if appHidden then
                    ...
                    if playBtn then playBtn.setEnabled(true); playBtn.setText("PLAY") end
                    if generateBtn then generateBtn.setEnabled(true); generateBtn.setText("REGENERATE PODCAST") end
                    showToast("Podcast ready (Direct mode)!")
                end
            end)
            isGenerating = false
            return
        ...
        local turn = dialogue[index]
        local speakerName = (turn.speaker == "host") and "Host" or "Guest"
        local voice = (turn.speaker == "host") and hostVoice or guestVoice
        runOnUi(function() if generateBtn then generateBtn.setText("Direct: " .. speakerName .. " " .. index .. "/" .. #dialogue) end end)
        fetchAudioSimple(turn.text, voice, apikey, model, function(success, audio, errMsg)
            if success then
                table.insert(allAudio, audio)
                index = index + 1
                delay(1000, processSegment)
            else
                runOnUi(function() showToast("Segment " .. index .. " failed, skipping") end)
                index = index + 1
                delay(1000, processSegment)
            end
        end)
...
function generateSingleAudio(text, voice, apikey, model, generateBtn, playBtn, resultLayout)
    if currentHttpRequest then pcall(function() currentHttpRequest.cancel() end) end
    ...
    if selectedEmotion ~= "Neutral" then
        local emotionMap = {
            ["Neutral"] = "", ["Happy"] = "Speak in a happy, cheerful tone: ", ["Sad"] = "Speak in a sad, melancholic tone: ",
            ["Angry"] = "Speak in an angry, aggressive tone: ", ["Excited"] = "Speak in an excited, energetic tone: ",
            ["Fearful"] = "Speak in a fearful, trembling tone: ", ["News Anchor"] = "Speak like a professional news anchor: ",
            ["Narrator"] = "Speak like a calm storyteller: ", ["Formal"] = "Speak in a formal, official tone: ",
            ["Casual"] = "Speak in a casual, informal tone: ", ["Friendly"] = "Speak in a warm, friendly tone: ",
            ["Cheerful"] = "Speak cheerfully: ", ["Hopeful"] = "Speak hopefully: ", ["Playful"] = "Speak playfully: ",
            ["Romantic"] = "Speak romantically: ", ["Loving"] = "Speak lovingly: ", ["Gentle"] = "Speak gently: ",
            ["Humble"] = "Speak humbly: ", ["Innocent"] = "Speak innocently: ", ["Calm"] = "Speak calmly: ",
            ["Confident"] = "Speak confidently: ", ["Proud"] = "Speak proudly: ", ["Strong"] = "Speak strongly: ",
            ["Bold"] = "Speak boldly: ", ["Passionate"] = "Speak passionately: ", ["Gloomy"] = "Speak gloomily: ",
            ["Desperate"] = "Speak desperately: ", ["Shy"] = "Speak shyly: ", ["Bored"] = "Speak in a bored tone: ",
            ["Surprised"] = "Speak with surprise: ", ["Sarcastic"] = "Speak sarcastically: ", ["Storytelling"] = "Speak like a storyteller: ",
            ["Dramatic"] = "Speak dramatically: ", ["Energetic"] = "Speak energetically: ", ["Funny"] = "Speak humorously: ",
            ["Mysterious"] = "Speak mysteriously: ", ["Serious"] = "Speak seriously: ", ["Whisper"] = "Whisper softly: ",
            ["Fast"] = "Speak quickly: ", ["Slow"] = "Speak slowly: "
        }
    ...
    local function makeRequest(attempt)
        attempt = attempt or 1
        currentHttpRequest = Http.post(apiUrl, cjson.encode(requestBody), headers, function(code, content)
            currentHttpRequest = nil
            if code == 200 then
                local ok, data = pcall(cjson.decode, content)
                if ok and data and data.candidates and #data.candidates > 0 then
                ...
                isGenerating = false
            elseif code == 429 and attempt < 3 then
                delay(3000, function() makeRequest(attempt+1) end)
            elseif attempt < 3 then
                delay(3000, function() makeRequest(attempt+1) end)
            else
                runOnUi(function()
...
function generateSingingChunkWithRetry(text, voice, apikey, model, maxRetries, callback, attempt)
        ...
        run = function()
            if not completed then
                ...
                if attempt < maxRetries then
                    delay(3000, function() generateSingingChunkWithRetry(text, voice, apikey, model, maxRetries, callback, attempt+1) end)
                else
    ...
    local http = Http.post(apiUrl, cjson.encode(requestBody), headers, function(code, content)
        ...
        elseif code == 429 and attempt < maxRetries then
            delay(4000, function() generateSingingChunkWithRetry(text, voice, apikey, model, maxRetries, callback, attempt+1) end)
...
function getBackgroundMusicPath(style, callback)
    ...
    if file.exists() then callback(localPath) return end
...
function generateMusicFromDialogue(dialogue, style, apikey, model, generateBtn, playBtn, resultLayout)
    ...
    local function processNext()
        if current > total then
            ...
            getBackgroundMusicPath(style, function(bgPath)
                if not bgPath then
                    runOnUi(function()
                        if generateBtn then generateBtn.setEnabled(true) end
        ...
        generateSingingChunkWithRetry(lyric, voice, apikey, model, 5, function(audio, err)
            if audio then
                table.insert(allAudio, audio)
                current = current + 1
                delay(800, processNext)
            else
...
• Select an emotion from 40+ styles
• Type your story/script
...
• Write your own lyrics OR enter a theme
• AI creates male+female singing duet

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ Text to Podcast Mode (TWO OPTIONS)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Select "Text to Podcast" from Mode dropdown
• Choose Host Voice and Guest Voice
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• While generating, tap HIDE
• Extension closes, generation continues
• When complete, app reopens automatically

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          SUPPORT & FEEDBACK
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Click "Send Feedback" to contact developer.
]]
    local views = {}
    local layout = {
...
function showMain()
        ...
        ScrollView, layout_width = "fill", layout_height = "fill", id = "windowBackground",
        {
            ...
            -- Podcast mode settings
            {
                LinearLayout, id = "podcastSettingsLayout", orientation = "vertical", layout_width = "fill", layout_height = "wrap", visibility = isPodcastMode and View.VISIBLE or View.GONE,
                {
                    ...
                    { TextView, text = "Podcast Mode", textSize = 12, textColor = "#CCCCCC", typeface = Typeface.DEFAULT_BOLD, layout_marginBottom = "6dp" },
                    { RadioGroup, id = "podcastModeGroup", layout_width = "fill", layout_height = "wrap", orientation = "horizontal", layout_marginBottom = "12dp",
                        { RadioButton, id = "aiModeRadio", text = "AI Generate Conversation", layout_width = "0dp", layout_weight = "1", textColor = "#FFFFFF" },
                        { RadioButton, id = "directModeRadio", text = "Direct Read as Duet", layout_width = "0dp", layout_weight = "1", textColor = "#FFFFFF" }
                    },
                    { TextView, text = "Playback Speed", textSize = 12, textColor = "#CCCCCC", typeface = Typeface.DEFAULT_BOLD },
    ...
    dlg.setOnDismissListener({ onDismiss = function() 
        if not hiddenGenerating then closeAndCleanup() end
    end })
    ...
    views.modeSpinner.onItemSelectedListener = {
        onItemSelected = function(p, v, pos, id)
            ...
            elseif pos == 1 then
                ...
                if podcastGenerationMode == 1 then views.aiModeRadio.setChecked(true) else views.directModeRadio.setChecked(true) end
            end
            saveSettings()
        end
    }
    ...
    if views.emotionSpin then
        ...
        views.emotionSpin.onItemSelectedListener = {
            onItemSelected = function(p, v, pos, id) selectedEmotion = EMOTIONS[pos+1]; saveSettings() end
        }
    end
    ...
    if views.aiModeRadio and views.directModeRadio then
        views.aiModeRadio.setChecked(podcastGenerationMode == 1)
        views.directModeRadio.setChecked(podcastGenerationMode == 2)
        views.aiModeRadio.setOnCheckedChangeListener({
            onCheckedChanged = function(btn, isChecked) if isChecked then podcastGenerationMode = 1; saveSettings() end end
        })
        views.directModeRadio.setOnCheckedChangeListener({
            onCheckedChanged = function(btn, isChecked) if isChecked then podcastGenerationMode = 2; saveSettings() end end
        })
    end
    ...
    views.generateBtn.onClick = function()
        ...
        if isMusicMode then
            ...
            if theme == "" and lyrics == "" then showToast("Enter theme or lyrics") return end
        else
        ...
        elseif isPodcastMode then
            if podcastGenerationMode == 2 then
                views.generateBtn.setText("DIRECT PODCAST...")
                generatePodcastDirect(userText, googleApiKey, selectedVoice, selectedVoice2, selectedModel, views.generateBtn, views.playBtn, views.resultLayout)
            else
                views.generateBtn.setText("AI PODCAST...")
    ...
    views.aboutBtn.onClick = function() vibrate(30); showAboutDialog() end
...
if googleApiKey == "" or firstRun then
    if firstRun then showApiSettings(true) else showApiSettings(false) end
else
    showMain()
end